...
Conclusion:
In this lab, we successfully implemented the Cohen–Sutherland Line Clipping Algorithm using the graphics.h library in C++. The program allowed us to input the coordinates of a viewport and a line, and determine whether the line was completely visible, partially visible, or completely invisible within the defined clipping window.

Through this implementation, we understood how region codes (outcodes) are assigned to endpoints and how logical operations are used to decide trivial acceptance or rejection of a line. For partially visible lines, intersection points were calculated using the slope equation, and the line was clipped accordingly.